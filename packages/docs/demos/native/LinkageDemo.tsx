...
function CityEditor({ value, onChange, country }: { value: unknown; onChange: (v: unknown) => void; country: string }) {
    ...
    return (
        <select className="et-editor-select" value={value as string} onChange={(e) => onChange(e.target.value)}>
...
export default function LinkageDemo() {
    ...
    const columns: EditableColumn<User>[] = [
        ...
        },
        {
            title: '是否需要电话',
            dataIndex: 'hasIphone',
            editRender: ({ value, onChange }) => <Switch checked={value} onChange={onChange} />,
        },
        {
            title: '电话',
            dataIndex: 'iphone',
            editRender: ({ value, onChange, row }) => {
                ...
                return <Input disabled={!row.hasIphone} value={value} onChange={(e) => onChange(e.target.value)} />;
            },
        },
    ];
    return (
