...
FormilyEditableTable 内部使用 Formily 2.x 标准的 `<ArrayField>` + `<Field>` 方案，完全兼容 Formily 的 effects 机制。
...
FormilyEditableTable 的每一行单元格都是一个标准的 Formily `<Field>`，由 Formily 管理值、校验和联动：

1. **行级 Field 上下文**：每个单元格的列 render 会自动包裹一层 `<Field name={rowIndex}>`，创建行级上下文
2. **单元格 Field**：`<FormilyEditableTable.Field name="type">` 内部渲染 `<Field name="type">`，自动解析为 `items.0.type`
...
FormilyEditableTable 中 effects 路径使用通配符 `*` 匹配行索引：

```
{name}.*.{fieldName}
 │       │   │
 │       │   └── FormilyEditableTable.Field 的 name 属性
 │       └────── 匹配所有行索引（0, 1, 2, ...）
 └────────────── FormilyEditableTable 的 name 属性
```
...
FormilyEditableTable 完全兼容 Formily 2.x 的 effects API。以下是与表格场景最常用的 effects 方法：
...
选择「类型」后，动态改变「子类型」的下拉选项。通过 `state.data` 传递动态 props（如 `options`），`FormilyEditableTable.Field` 会自动合并到子组件上：
...
`state.component = [Select, { options }]` 会导致 Formily 用 `state.component` 替换整个子组件渲染，丢失原始 JSX 上的 `placeholder`、`style` 等属性。使用 `state.data` 传递动态 props，`FormilyEditableTable.Field` 会将其合并到子组件上，保留原始属性。
:::
...
配合 `FormilyEditableTable.Field` 的 `editable` 属性，`state.editable = false` 时字段会显示为只读文本。
